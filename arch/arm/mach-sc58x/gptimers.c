...
void set_gptimer_pwidth(struct sc58x_gptimer *timer, uint32_t value)
{
	writel(value, timer->io_base + GPTIMER_WID_OFF);
}
EXPORT_SYMBOL(set_gptimer_pwidth);

void set_gptimer_period(struct sc58x_gptimer *timer, uint32_t period)
{
	writel(period, timer->io_base + GPTIMER_PER_OFF);
}
EXPORT_SYMBOL(set_gptimer_period);

uint32_t get_gptimer_count(struct sc58x_gptimer *timer)
{
...
void set_gptimer_config(struct sc58x_gptimer *timer, uint16_t config)
{
...
struct sc58x_gptimer *gptimer_request(int id)
{
	struct sc58x_gptimer *t = NULL;
	unsigned long flags;
	int found;
...
int gptimer_free(struct sc58x_gptimer *timer)
{
	if (!timer)
		return -EINVAL;
...
static int sc58x_gptimer_probe(struct platform_device *pdev)
{
	unsigned long flags;
	struct sc58x_gptimer *timer;
	int irq;
	struct resource *mem;
	...
	timer = devm_kzalloc(dev, sizeof(struct sc58x_gptimer), GFP_KERNEL);
	if (!timer) {
...
static int sc58x_gptimer_remove(struct platform_device *pdev)
{
	struct sc58x_gptimer *timer;
	unsigned long flags;
	int ret = -EINVAL;
...
static const struct of_device_id sc58x_gptimer_match[] = {
	{
		.compatible = "adi,sc58x-timer",
	},
	{},
};
MODULE_DEVICE_TABLE(of, sc58x_gptimer_match);

static struct platform_driver sc58x_gptimer_driver = {
	.probe = sc58x_gptimer_probe,
	.remove = sc58x_gptimer_remove,
	.driver = {
		.name = "sc58x_gptimer",
		.of_match_table = of_match_ptr(sc58x_gptimer_match),
	},
};

static int __init sc58x_gptimer_init(void)
{
	return platform_driver_register(&sc58x_gptimer_driver);
}
arch_initcall(sc58x_gptimer_init);

static void __exit sc58x_gptimer_exit(void)
{
	platform_driver_unregister(&sc58x_gptimer_driver);
}
