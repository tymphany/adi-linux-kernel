...
/*
 * All ADI SC5XX system MMRs are padded to 32bits even if the register
 * itself is only 16bits.  So use a helper macro to streamline this.
 */
#define __ADI(m) u16 m; u16 __pad_##m

/*
 * adi ppi registers layout
 */
struct adi_ppi_regs {
	__ADI(control);
	__ADI(status);
...
/*
 * adi eppi registers layout
 */
struct adi_eppi_regs {
	__ADI(status);
	__ADI(hcount);
	__ADI(hdelay);
	__ADI(vcount);
...
/*
 * adi eppi3 registers layout
 */
struct adi_eppi3_regs {
	u32 stat;
	u32 hcnt;
	u32 hdly;
	u32 vcnt;
