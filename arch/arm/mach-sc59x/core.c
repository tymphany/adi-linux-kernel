 ...
 */
void __init sc59x_timer_init(void)
{
	struct device_node *np, *clocksrc_np = NULL, *clockevent_np = NULL;

	for_each_compatible_node(np, NULL, "adi,sc59x-timer-core") {
		if (!clocksrc_np &&
	...
	timer_clock = sc59x_timer_of_init(clocksrc_np);
