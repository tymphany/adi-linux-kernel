...
static int adi_crypt_prepare_req(struct skcipher_request *req)
{
	...
	if (crypt->flags & PKTE_TCM_MODE) {
		dev_err(crypt->dev, "%s: PKTE_TCM_MODE Currently untested and unsupported", __func__);
		return -EINVAL;
	}

	if (crypt->flags & PKTE_HOST_MODE) {
		dev_err(crypt->dev, "%s: PKTE_HOST_MODE Currently untested and unsupported", __func__);
		return -EINVAL;
	}
	...
	if (!(ctx->flags_skcipher & PKTE_FLAGS_STARTED)) {
		...
		if (rctx->mode & ENCRYPT) {
			crypt->pkte_device->pPkteList.pCommand.opcode = opcode_encrypt;
			crypt->pkte_device->pPkteList.pCommand.direction = dir_outbound;
		} else if (rctx->mode & DECRYPT) {
			crypt->pkte_device->pPkteList.pCommand.opcode = opcode_decrypt;
			crypt->pkte_device->pPkteList.pCommand.direction = dir_inbound;
		} else {
		...
		if (rctx->mode & CBC) {
			...
			crypt->pkte_device->pPkteList.pCommand.cipher_mode = cipher_mode_ecb;
		}
		...
		crypt->pkte_device->pPkteList.pCommand.hash_source = hash_source_no_load;
		crypt->pkte_device->pPkteList.pCommand.final_hash_condition = final_hash;

		adi_reset_state(crypt);
		adi_start_engine(crypt);
	}

	crypt->pkte_device->pPkteList.pSource = &crypt->pkte_device->source[crypt->ring_pos_consume][0];

	if (crypt->flags & (PKTE_TCM_MODE | PKTE_AUTONOMOUS_MODE)) {
		adi_configure_cdr(crypt);
	}

	crypt->pkte_device->pPkteList.pDestination = &crypt->pkte_device->destination[0];
	...
	for (i = 0; i < req->cryptlen/4; i++) {
		dev_dbg(crypt->dev, "SRC %x\n", crypt->pkte_device->source[crypt->ring_pos_consume][i]);
	}
#endif
