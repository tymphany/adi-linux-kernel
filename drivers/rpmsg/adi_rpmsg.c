...
#define ADI_RESOURCE_TABLE_INIT_TIMEOUT_MS (0)
...
static bool adi_rpmsg_notify(struct virtqueue *vq)
{
	...
	if(rpchan->rpmsg_state == ADI_RP_RPMSG_WAITING) {
		...
		if(rpchan->adi_rsc_table->adi_table_hdr.initialized == ADI_RESOURCE_TABLE_INIT_MAGIC) {
			...
			dev_info(rpchan->dev, "Core%d resource table not initialized, delay first notify\n", rpchan->core_id);
			if(ADI_RESOURCE_TABLE_INIT_TIMEOUT_MS == 0) {
				/* Wait forever */
				step = 0;
				dev_info(rpchan->dev, "Wait forever for Core%d resource table init\n", rpchan->core_id);
			} else {
				step = __ADI_DELAY_MS;
			}
			...
			if (rpchan->rpmsg_state != ADI_RP_RPMSG_SYNCED) {
				...
				dev_info(rpchan->dev, "Core%d rpmsg init timeout, probably not supported.\n", rpchan->core_id);
			}
		}
	}
				    ...
				    const char *name, bool ctx)
{
	...
	vq = vring_new_virtqueue(id, num, align, vdev, false, ctx, addr, adi_rpmsg_notify, callback, name);
	if (!vq) {
...
static int adi_rpmsg_probe(struct platform_device *pdev)
{
	...
	rpchan->icc_irq_flags = (res->flags & IORESOURCE_BITS) | IRQF_PERCPU | IRQF_SHARED | IRQF_ONESHOT;
	...
	if(node){
		...
		of_node_put(node);
		if (!rmem) {
			dev_err(dev, "Faied to acquire vdev-vring\n");
			ret = -EINVAL;
			goto free_adi_rcu;
		}

		if(rmem->size < 0x4000){
			dev_err(dev, "Not enough space in vdev-vring, minimal space required is 16kB\n");
			ret = -EINVAL;
			goto free_adi_rcu;
		}
		...
		rpchan->vring[0].size = PAGE_ALIGN(vring_size(rpchan->rsc_vring[0]->num, rpchan->rsc_vring[0]->align));
		rpchan->vring[1].size = PAGE_ALIGN(vring_size(rpchan->rsc_vring[1]->num, rpchan->rsc_vring[1]->align));
		va = dma_alloc_coherent(dev, rpchan->vring[0].size + rpchan->vring[1].size, &dma, GFP_KERNEL);
		if(!(va)) {
			dev_err(dev, "Unable to map vdev-vring\n");
			ret = -ENOMEM;
			goto free_adi_rcu;
