 ...
 *
 * Copyright 2005-2018 Analog Devices Inc.
 *
	...
	if (twi_int_status & MCOMP) {
		if (twi_int_status & (XMTSERV | RCVSERV) &&
			...
			if (iface->readNum == 0) {
				 ...
				 * combine mode.
				 */
				writeValue = (ioread16(&iface->regs_base->master_ctl) & (~(0xff << 6))) | (iface->readNum << 6);
				iowrite16(writeValue, &iface->regs_base->master_ctl);
			}
			...
			if (iface->pmsg[iface->cur_msg].len <= 255) {
				writeValue = (ioread16(&iface->regs_base->master_ctl) & (~(0xff << 6))) | (iface->pmsg[iface->cur_msg].len << 6);
				iowrite16(writeValue, &iface->regs_base->master_ctl);
				iface->manual_stop = 0;
			} else {
				writeValue = (ioread16(&iface->regs_base->master_ctl) | (0xff << 6));
			...
			bool polling)
{
	...
	case TWI_I2C_MODE_STANDARDSUB:
		...
		writeValue = MCOMP | MERR | ((iface->read_write == I2C_SMBUS_READ) ? RCVSERV : XMTSERV);
		...
		/* Master enable */
		writeValue = ioread16(&iface->regs_base->master_ctl) | MEN | ((iface->twi_clk > 100) ? FAST : 0);
		iowrite16(writeValue, &iface->regs_base->master_ctl);
		break;
	case TWI_I2C_MODE_COMBINED:
		...
		/* Master enable */
		writeValue = ioread16(&iface->regs_base->master_ctl) | MEN | RSTART | ((iface->twi_clk > 100) ? FAST : 0);
		iowrite16(writeValue, &iface->regs_base->master_ctl);
		break;
	default:
		iowrite16(0, &iface->regs_base->master_ctl);
			...
			if (iface->read_write != I2C_SMBUS_READ) {
				if (iface->writeNum > 0) {
					...
					if (iface->writeNum <= 255)
						iowrite16(iface->writeNum << 6, &iface->regs_base->master_ctl);
					else {
						iowrite16(0xff << 6, &iface->regs_base->master_ctl);
						iface->manual_stop = 1;
					}
				...
				if (iface->readNum > 0 && iface->readNum <= 255)
					iowrite16(iface->readNum << 6, &iface->regs_base->master_ctl);
...
#if defined(CONFIG_ARCH_SC59X_64) || defined(CONFIG_ARCH_SC58X) || defined(CONFIG_ARCH_SC59X)
