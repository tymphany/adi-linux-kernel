...
static const char *can_sels[] = {"oclk_0", "oclk_1", "dclk_1", "oclk_0_half"};
static const char *spdif_sels[] = {"oclk_0", "oclk_1", "dclk_1", "dclk_0"};
static const char *gige_sels[] = {"sclk1_0", "sclk1_1", "cclk0_1", "oclk_0"};
...
static int cdu_check_clocks(struct device *dev, struct clk *clks[], size_t count) {
	...
	for (i = 0; i < count; ++i) {
		if (IS_ERR(clks[i])) {
			dev_err(dev, "Clock %zu failed to register: %ld\n", i, PTR_ERR(clks[i]));
...
static int sc57x_clock_probe(struct platform_device *pdev) {
	...
	clks[ADSP_SC57X_CLK_DUMMY] = clk_register_fixed_rate(dev, "dummy", NULL, 0, 0);
	...
	clks[ADSP_SC57X_CLK_CGU1_IN] = clk_register_mux(dev, "cgu1_in_sel",
		cgu1_in_sels, 2, CLK_SET_RATE_PARENT, cdu + CDU_CLKINSEL, 0, 1, 0,
		&cdu_lock);

	// CGU configuration and internal clocks
	clks[ADSP_SC57X_CLK_CGU0_PLL_IN] = clk_register_divider(dev, "cgu0_df",
		"sys_clkin0", CLK_SET_RATE_PARENT, cgu0 + CGU_CTL, 0, 1, 0, &cdu_lock);
	clks[ADSP_SC57X_CLK_CGU1_PLL_IN] = clk_register_divider(dev, "cgu1_df",
		"cgu1_in_sel", CLK_SET_RATE_PARENT, cgu1 + CGU_CTL, 0, 1, 0, &cdu_lock);

	// VCO output == PLL output
	clks[ADSP_SC57X_CLK_CGU0_PLLCLK] = sc5xx_cgu_pll(dev, "cgu0_pllclk", "cgu0_df",
		cgu0 + CGU_CTL, CGU_MSEL_SHIFT, CGU_MSEL_WIDTH, 0, false, &cdu_lock);
	clks[ADSP_SC57X_CLK_CGU1_PLLCLK] = sc5xx_cgu_pll(dev, "cgu1_pllclk", "cgu1_df",
		cgu1 + CGU_CTL, CGU_MSEL_SHIFT, CGU_MSEL_WIDTH, 0, false, &cdu_lock);

	// Dividers from pll output
	clks[ADSP_SC57X_CLK_CGU0_CDIV] = cgu_divider(dev, "cgu0_cdiv", "cgu0_pllclk",
		cgu0 + CGU_DIV, 0, 5, 0);
	clks[ADSP_SC57X_CLK_CGU0_SYSCLK] = cgu_divider(dev, "sysclk_0", "cgu0_pllclk",
		cgu0 + CGU_DIV, 8, 5, 0);
	clks[ADSP_SC57X_CLK_CGU0_DDIV] = cgu_divider(dev, "cgu0_ddiv", "cgu0_pllclk",
		cgu0 + CGU_DIV, 16, 5, 0);
	clks[ADSP_SC57X_CLK_CGU0_ODIV] = cgu_divider(dev, "cgu0_odiv", "cgu0_pllclk",
		cgu0 + CGU_DIV, 22, 7, 0);
	clks[ADSP_SC57X_CLK_CGU0_S0SELDIV] = cgu_divider(dev, "cgu0_s0seldiv",
		"sysclk_0", cgu0 + CGU_DIV, 5, 3, 0);
	clks[ADSP_SC57X_CLK_CGU0_S1SELDIV] = cgu_divider(dev, "cgu0_s1seldiv",
		"sysclk_0", cgu0 + CGU_DIV, 13, 3, 0);

	clks[ADSP_SC57X_CLK_CGU1_CDIV] = cgu_divider(dev, "cgu1_cdiv", "cgu1_pllclk",
		cgu1 + CGU_DIV, 0, 5, 0);
	clks[ADSP_SC57X_CLK_CGU1_SYSCLK] = cgu_divider(dev, "sysclk_1", "cgu1_pllclk",
		cgu1 + CGU_DIV, 8, 5, 0);
	clks[ADSP_SC57X_CLK_CGU1_DDIV] = cgu_divider(dev, "cgu1_ddiv", "cgu1_pllclk",
		cgu1 + CGU_DIV, 16, 5, 0);
	clks[ADSP_SC57X_CLK_CGU1_ODIV] = cgu_divider(dev, "cgu1_odiv", "cgu1_pllclk",
		cgu1 + CGU_DIV, 22, 7, 0);
	clks[ADSP_SC57X_CLK_CGU1_S0SELDIV] = cgu_divider(dev, "cgu1_s0seldiv",
		"sysclk_1", cgu1 + CGU_DIV, 5, 3, 0);
	clks[ADSP_SC57X_CLK_CGU1_S1SELDIV] = cgu_divider(dev, "cgu1_s1seldiv",
		"sysclk_1", cgu1 + CGU_DIV, 13, 3, 0);

	// Gates to enable CGU outputs
	clks[ADSP_SC57X_CLK_CGU0_CCLK0] = cgu_gate(dev, "cclk0_0", "cgu0_cdiv",
		cgu0 + CGU_CCBF_DIS, 0);
	clks[ADSP_SC57X_CLK_CGU0_CCLK1] = cgu_gate(dev, "cclk1_0", "cgu0_cdiv",
		cgu1 + CGU_CCBF_DIS, 1);
	clks[ADSP_SC57X_CLK_CGU0_OCLK] = cgu_gate(dev, "oclk_0", "cgu0_odiv",
		cgu0 + CGU_SCBF_DIS, 3);
	clks[ADSP_SC57X_CLK_CGU0_DCLK] = cgu_gate(dev, "dclk_0", "cgu0_ddiv",
		cgu0 + CGU_SCBF_DIS, 2);
	clks[ADSP_SC57X_CLK_CGU0_SCLK1] = cgu_gate(dev, "sclk1_0", "cgu0_s1seldiv",
		cgu0 + CGU_SCBF_DIS, 1);
	clks[ADSP_SC57X_CLK_CGU0_SCLK0] = cgu_gate(dev, "sclk0_0", "cgu0_s0seldiv",
		cgu0 + CGU_SCBF_DIS, 0);

	clks[ADSP_SC57X_CLK_CGU1_CCLK0] = cgu_gate(dev, "cclk0_1", "cgu1_cdiv",
		cgu1 + CGU_CCBF_DIS, 0);
	clks[ADSP_SC57X_CLK_CGU1_CCLK1] = cgu_gate(dev, "cclk1_1", "cgu1_cdiv",
		cgu1 + CGU_CCBF_DIS, 1);
	clks[ADSP_SC57X_CLK_CGU1_OCLK] = cgu_gate(dev, "oclk_1", "cgu1_odiv",
		cgu1 + CGU_SCBF_DIS, 3);
	clks[ADSP_SC57X_CLK_CGU1_DCLK] = cgu_gate(dev, "dclk_1", "cgu1_ddiv",
		cgu1 + CGU_SCBF_DIS, 2);
	clks[ADSP_SC57X_CLK_CGU1_SCLK1] = cgu_gate(dev, "sclk1_1", "cgu1_s1seldiv",
		cgu1 + CGU_SCBF_DIS, 1);
	clks[ADSP_SC57X_CLK_CGU1_SCLK0] = cgu_gate(dev, "sclk0_1", "cgu1_s0seldiv",
		cgu1 + CGU_SCBF_DIS, 0);

	// Extra half rate clocks generated in the CDU
	clks[ADSP_SC57X_CLK_OCLK0_HALF] = clk_register_fixed_factor(dev, "oclk_0_half",
		"oclk_0", CLK_SET_RATE_PARENT, 1, 2);
	clks[ADSP_SC57X_CLK_CCLK1_1_HALF] = clk_register_fixed_factor(dev, "cclk1_1_half",
		"cclk1_1", CLK_SET_RATE_PARENT, 1, 2);

	// CDU output muxes
	clks[ADSP_SC57X_CLK_SHARC0_SEL] = cdu_mux(dev, "sharc0_sel", cdu + CDU_CFG0,
		sharc0_sels);
	clks[ADSP_SC57X_CLK_SHARC1_SEL] = cdu_mux(dev, "sharc1_sel", cdu + CDU_CFG1,
		sharc1_sels);
	clks[ADSP_SC57X_CLK_ARM_SEL] = cdu_mux(dev, "arm_sel", cdu + CDU_CFG2,
		arm_sels);
	clks[ADSP_SC57X_CLK_CDU_DDR_SEL] = cdu_mux(dev, "cdu_ddr_sel", cdu + CDU_CFG3,
		cdu_ddr_sels);
	clks[ADSP_SC57X_CLK_CAN_SEL] = cdu_mux(dev, "can_sel", cdu + CDU_CFG4,
		can_sels);
	clks[ADSP_SC57X_CLK_SPDIF_SEL] = cdu_mux(dev, "spdif_sel", cdu + CDU_CFG5,
		spdif_sels);
	clks[ADSP_SC57X_CLK_GIGE_SEL] = cdu_mux(dev, "gige_sel", cdu + CDU_CFG7,
		gige_sels);
	clks[ADSP_SC57X_CLK_SDIO_SEL] = cdu_mux(dev, "sdio_sel", cdu + CDU_CFG9, sdio_sels);

	// CDU output enable gates
	clks[ADSP_SC57X_CLK_SHARC0] = cdu_gate(dev, "sharc0", "sharc0_sel",
		cdu + CDU_CFG0, CLK_IS_CRITICAL);
	clks[ADSP_SC57X_CLK_SHARC1] = cdu_gate(dev, "sharc1", "sharc1_sel",
		cdu + CDU_CFG1, CLK_IS_CRITICAL);
	clks[ADSP_SC57X_CLK_ARM] = cdu_gate(dev, "arm", "arm_sel", cdu + CDU_CFG2,
		CLK_IS_CRITICAL);
	clks[ADSP_SC57X_CLK_CDU_DDR] = cdu_gate(dev, "cdu_ddr", "cdu_ddr_sel",
		cdu + CDU_CFG3, CLK_IS_CRITICAL);
	clks[ADSP_SC57X_CLK_CAN] = cdu_gate(dev, "can", "can_sel", cdu + CDU_CFG4, 0);
	clks[ADSP_SC57X_CLK_SPDIF] = cdu_gate(dev, "spdif", "spdif_sel", cdu + CDU_CFG5,
		0);
	clks[ADSP_SC57X_CLK_GIGE] = cdu_gate(dev, "gige", "gige_sel", cdu + CDU_CFG7, 0);
	clks[ADSP_SC57X_CLK_SDIO] = cdu_gate(dev, "sdio", "sdio_sel", cdu + CDU_CFG9, 0);
...
static const struct of_device_id sc57x_clk_of_match[] = {
	{ .compatible = "adi,sc57x-clocks" },
	{ },
};
MODULE_DEVICE_TABLE(of, sc57x_clk_of_match);

static struct platform_driver sc57x_clk_driver = {
	.probe = sc57x_clock_probe,
	.driver = {
		.name = "sc57x-clocks",
		.of_match_table = sc57x_clk_of_match,
	},
};
module_platform_driver(sc57x_clk_driver);
