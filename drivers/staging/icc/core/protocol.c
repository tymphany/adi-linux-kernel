...
static struct sm_message *get_message_from_tx_list_by_payload(
	struct sm_session *session, uint32_t payload)
{
...
static int sm_query_remote_ep(uint32_t dst_ep, uint32_t dst_cpu,
	int timeout, int nonblock)
{
	...
	if (nonblock) {
		...
		ret = wait_event_interruptible_timeout(
					adi_icc->sessions_table->query_wait,
...
static int
sm_wait_request_finish(uint32_t session_idx, void *user_buf, uint32_t dst_cpu,
	int timeout, uint32_t request_type, uint32_t *buf_len,
	uint32_t payload, int nonblock)
{
	int ret = 0;
	struct sm_message *message;
	struct sm_session *session;
	...
	if (nonblock) {
		if (request_type == WAIT_SEND) {
			message =
				get_message_from_tx_list_by_payload(session, payload);
			if (message == NULL)
				return -EINVAL;
			if (message->b_tx_finish) {
				ret = 0;
				*buf_len = message->msg.length;
				mutex_lock(&adi_icc->sessions_table->lock);
				list_del(&message->next);
			...
			if (session->b_rx_finish) {
				...
				if (message == NULL)
					return -EINVAL;
				ret = fill_user_buffer_from_message(
							session, message, user_buf, buf_len);
				mutex_lock(&adi_icc->sessions_table->lock);
				list_del(&message->next);
				if (message)
					kfree(message);
		...
		if (request_type == WAIT_SEND) {
			...
			if (!message->b_tx_finish) {
				...
				ret = wait_event_interruptible_timeout(
						message->icc_info->iccq_tx_wait,
						message->b_tx_finish == 1,
						timeout);
				if (!ret) {
...
sm_default_recvmsg(struct sm_msg *msg, struct sm_session *session)
{
	...
	case SM_SESSION_PACKET_CONSUMED:
		uncompleted =
			get_message_from_tx_list_by_payload(session, msg->payload);
		if (uncompleted == NULL)
			return -EINVAL;
		mutex_lock(&table->lock);
...
static int adi_icc_probe(struct platform_device *pdev)
{
	...
	if (!icc) {
		dev_err(&pdev->dev, "fail to malloc adi_icc\n");
		return -ENOMEM;
	}
	...
	icc->base = devm_ioremap_nocache(dev, res->start, resource_size(res));
...
#define PEERINFO_CELLS	2
		...
		if (!icc->icc_info) {
			dev_err(dev, "fail to malloc icc_info array\n");
			return -ENOMEM;
		}
