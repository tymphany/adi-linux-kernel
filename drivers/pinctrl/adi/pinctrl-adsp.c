...
/* does not need lock */
static void adsp_set_pin_gpio(struct adsp_gpio_port *port, unsigned offset, bool gpio) {
	if (gpio)
 ...
 */
static void adsp_portmux_setup(struct adsp_gpio_port *port, unsigned offset,
...
static int adsp_pinctrl_get_group_pins(struct pinctrl_dev *pctldev, unsigned selector,
